...
function loadData() {
  try {
    return JSON.parse(localStorage.getItem(STORAGE_KEY)) || { profile: null, plan: [], log: [], checkins: [], assessments: [], adaptationPrompts: {} };
  } catch { return { profile: null, plan: [], log: [], checkins: [], assessments: [], adaptationPrompts: {} }; }
}
...
function shouldRebuildSavedPlan(data) {
  ...
  if ((data.profile.planVersion || 0) < PLAN_SCHEMA_VERSION) return true;
  return data.plan.some(weekHasMalformedVolume);
}
...
function rebuildStoredPlan(data) {
  ...
  const rebuilt = buildPlan(preservedProfile);
  return {
    ...data,
...
function renderHeroPanel() {
  ...
  const phaseText = week ? `${week.phaseLabel}` : '建立你的個人手冊';
  ...
  const weeklyTarget = week?.targetKm ? `${week.targetKm} km` : '依可訓練日生成';
  shell.innerHTML = `
    <div class="trainer-hero-layout">
      <div class="trainer-hero-main">
        <div class="trainer-hero-kicker">Runner Planner</div>
        <h1 class="trainer-hero-title" id="trainer-hero-title">${hasPlan ? `${meta.label} 訓練手冊` : '把訓練變成真的能照著跑的手冊'}</h1>
        <p class="trainer-hero-copy">${hasPlan ? `${meta.handbook} 這週目前落在「${phaseText}」，課表會把主課、長跑、恢復跑與休息日分開安排。` : '不是只排距離，而是一起考慮每週訓練天數、長跑上限、恢復節奏與近期能力，生成更像真人教練在帶的週計畫。'}</p>
        <div class="trainer-hero-stats">
          <div class="trainer-stat">
            <div class="trainer-stat-label">目前模式</div>
            <div class="trainer-stat-value">${meta.label}</div>
            <div class="trainer-stat-sub">${hasPlan ? meta.cardDesc : '入門 / 半馬 / 全馬 / 傷後重建'}</div>
          </div>
          <div class="trainer-stat">
            <div class="trainer-stat-label">本週進度</div>
            <div class="trainer-stat-value">${progressText}</div>
            <div class="trainer-stat-sub">${hasPlan ? phaseText : '先完成設定後開始'}</div>
          </div>
          <div class="trainer-stat">
            <div class="trainer-stat-label">本週目標</div>
            <div class="trainer-stat-value">${weeklyTarget}</div>
            <div class="trainer-stat-sub">${hasPlan ? `目標配速 ${paceText}` : '會依你的條件自動估算'}</div>
          </div>
        </div>
        <div class="trainer-hero-points">
          <span class="trainer-point">目標日 ${targetDateText}</span>
          <span class="trainer-point">${hasPlan ? '休息日會輪替肌力與恢復' : '每週檢查一次就能微調'}</span>
          <span class="trainer-point">${hasPlan ? '圖解改成先看清單再對圖' : '熱身・主課・收操會分段顯示'}</span>
        </div>
      </div>
      <aside class="trainer-hero-side" aria-label="功能重點">
        <div class="trainer-side-card">
          <div class="trainer-side-kicker">This Week</div>
          <div class="trainer-side-title">${hasPlan ? '先看今天卡片，再用週評估修正方向' : '先建立你的模式，再讓系統開始排週計畫'}</div>
          <div class="trainer-side-copy">${hasPlan ? '每天只要照今天那張卡執行。真的需要調整時，用週評估和完成紀錄去修，不需要每天填一大堆表單。' : '設定目標、訓練日、週跑量與配速後，系統才會把主課、長跑、恢復跑和休息日拆開安排。'}</div>
          <div class="trainer-side-metric">${hasPlan ? `本週階段：${phaseText}` : '建立後才會出現本週資訊'}</div>
        </div>
        <div class="trainer-side-row">
          <div class="trainer-side-card">
            <div class="trainer-side-kicker">Quick View</div>
            <div class="trainer-side-title">你會看到什麼</div>
            <div class="trainer-side-list">
              <div class="trainer-side-item"><b>主課</b><span>${hasPlan ? '依模式與週期改變' : '節奏 / 長跑 / 恢復分流'}</span></div>
              <div class="trainer-side-item"><b>圖解</b><span>先看清單，再對教學海報</span></div>
            </div>
          </div>
          <div class="trainer-side-card">
            <div class="trainer-side-kicker">Adjustment</div>
            <div class="trainer-side-title">怎麼微調</div>
            <div class="trainer-side-list">
              <div class="trainer-side-item"><b>評估頻率</b><span>每週一次</span></div>
              <div class="trainer-side-item"><b>調整依據</b><span>完成率、疲勞、近期檢測</span></div>
            </div>
          </div>
        </div>
      </aside>
...
const GUIDE_LIBRARY = {
  warmup: {
    title: '熱身圖解',
    intro: '跑前先把關節打開、腿部喚醒，再進入主課，比直接開跑穩很多。',
    cover: {
  ...
  cooldown: {
    title: '收操伸展',
    intro: '跑完先回收呼吸，再把最容易緊的部位放鬆掉，隔天恢復差很多。',
    cover: {
  ...
  strength: {
    title: '肌力動作',
    intro: '休息日或恢復日做一張即可。A 偏臀腿穩定，B 偏核心與足踝，兩套輪流用比較像真人教練安排。',
    cover: {
...
function buildWorkoutPattern(profile, numTrain, weekNum, phaseName, isDeload, hasInjury, isEarlyBeginner, weekStart) {
  const rule = GOAL_RULES[profile.goal] || GOAL_RULES.half;
  const qualityReady = weekNum >= rule.qualityAfterWeeks && !isDeload && !hasInjury && !isEarlyBeginner;
  ...
  if (profile.goal === '5k10k') {
    const entryQuality = qualityType === 'interval'
      ...
      : qualityType === 'tempo'
        ? { type: 'tempo', focus: rotation % 2 === 0 ? 'steady' : 'progression', label: rotation % 2 === 0 ? '穩定節奏課' : '漸進節奏課' }
  ...
  if (numTrain === 3) {
    ...
    return rotation % 2 === 0
      ? [
          { type: qualityType, focus: qualityFocus, label: qualityType === 'interval' ? '本週主課' : '本週節奏主課' },
...
function buildWorkoutContent(type, km, profile, phaseName, weekNum, isDeload, isTaper, focus = '', label = '') {
  ...
  const fastEasy = profile.easyPaceSec > 15 ? secToPace(profile.easyPaceSec - 15) : easy;
  ...
  if (type === 'tempo') {
    ...
    const variant = isDeload ? variants[0] : focusMap[focus] || variants[phaseBucket === 'peak' ? 1 : rotation % variants.length];
    if (focus === 'marathon') {
  ...
  const longVariant = isDeload || isTaper ? longVariants[taperIndex] : longVariants[phaseBucket === 'peak' ? Math.min(1, longVariants.length - 1) : rotation % longVariants.length];
  return longVariant;
}
  ...
  if (kind === 'strength') {
    const strengthVariants = hasInjury ? [
    ...
    ] : [
      {
        ...
        type: '休息日訓練',
        dose: '20–30 分',
        detail: rotation === 0 ? '今天偏臀腿穩定，重點是把支撐做紮實。' : rotation === 1 ? '今天偏核心與單腳控制，避免跑到後段姿勢散掉。' : '今天偏足踝與臀中肌，讓落地更穩定。',
        drills: rotation === 0
          ? [
              { icon: '🌉', visual: 'bridge', name: '臀橋', dose: '3×12' },
              { icon: '↗️', visual: 'lunge', name: '分腿蹲', dose: '3×8 / 邊' },
              { icon: '🧍', visual: 'single_balance', name: '單腳站立', dose: '3×30 秒 / 邊' }
            ]
          : rotation === 1 ? [
              { icon: '🪵', visual: 'plank', name: '棒式', dose: '3×45 秒' },
              { icon: '🦪', visual: 'clam', name: '蚌殼式', dose: '3×15 / 邊' },
              { icon: '🧍', visual: 'single_balance', name: '單腳硬舉', dose: '2×8 / 邊' }
            ]
          : [
              { icon: '🦶', visual: 'single_balance', name: '提踵', dose: '3×15' },
              { icon: '🪑', visual: 'squat', name: '椅子坐站', dose: '2×12' },
              { icon: '↔️', visual: 'clam', name: '側向開腿', dose: '2×12 / 邊' }
            ]
      },
      {
        icon: '🧘',
...
function buildDayCard(dow, dateStr, type, km, profile, isDeload, isTaper, hasInjury, today, weekNum = 1, phaseName = 'build', focus = '', label = '') {
  ...
  card.pace = content.pace;
  card.steps = content.steps;
...
function buildWeekDays(profile, trainDows, longDow, otherDows, targetKm, isDeload, isTaper, hasInjury, weekNum, startDate, phaseName) {
  const days = [];
  const weekStart = new Date(startDate);
  weekStart.setDate(startDate.getDate() + (weekNum - 1) * 7 - startDate.getDay());
  ...
  let otherTypes = buildWorkoutPattern(profile, numTrain, weekNum, phaseName, isDeload, hasInjury, isEarlyBeginner, weekStart);
  if (hasInjury) {
    otherTypes = otherTypes.map(session => session.type === 'tempo' || session.type === 'interval'
      ? { ...session, type: 'easy', focus: 'recovery', label: '恢復跑' }
      : session);
  }

  let otherIdx = 0;
  const today = todayStr();
  const longKm = profile.dayState.includes(2)
  ...
  for (let dow = 0; dow < 7; dow++) {
    ...
    if (profile.dayState[dow] === 2) {
      ...
      days.push(buildRestDayCard(dow, dateStr, profile, today, weekNum));
    }
  }
...
function buildPlan(profile) {
  ...
  let baseKm = Math.min(Math.max(profile.weeklyKm * 1.03, 10), rule.maxWeeklyKm);
  ...
  phases.forEach(phase => {
    for (let wi = 0; wi < phase.weeks; wi++) {
      ...
      const isDeload = weekNum % 4 === 0 && !isTaper && weekNum < totalWeeks;
      ...
      plan.push({
        ...
        days: buildWeekDays(profile, trainDows, longDow, otherDows, targetKm, isDeload, isTaper, hasInjury, weekNum, startDate, phase.name)
      });

      if (!isDeload && !isTaper) {
        baseKm = Math.min(baseKm * (1 + rule.weeklyGrowth), rule.maxWeeklyKm);
      }
    }
  });
...
<div id="plan-tab-week" class="container">
  ${renderProgressCard(profile, plan)}
...
<div class="export-bar">
  ...
  <button class="btn btn-secondary" onclick="exportHTML()">📱 下載手機訓練手冊</button>
</div>`;
}
...
function renderProgressCard(profile, plan) {
  ...
  const totalKm = log.reduce((sum, entry) => sum + (entry.actualKm || 0), 0);
  const doneCount = log.length;
  const currWeekPlan = plan[currentWeek - 1];
  const currWeekDone = log.filter(entry => {
    const weekNum = Math.floor((new Date(entry.date) - new Date(profile.generatedAt)) / 86400000 / 7) + 1;
    return weekNum === currentWeek;
  }).reduce((sum, entry) => sum + (entry.actualKm || 0), 0);
  const plannedSessions = plan.slice(0, currentWeek).reduce((sum, week) => sum + week.days.filter(day => day.type !== 'rest').length, 0);
  const adherence = plannedSessions > 0 ? Math.round((doneCount / plannedSessions) * 100) : 0;
...
<div class="card">
  ...
  <div style="font-size:18px;font-weight:700;margin-bottom:4px">${GOAL_NAME[profile.goal]} 訓練計畫</div>
  <div style="font-size:13px;color:var(--c-text-muted);margin-bottom:12px">
    目標日 ${profile.targetDate} · 目標配速 ${secToPace(profile.racePaceSec)}/km
  </div>
  <div class="mode-summary">
    <div class="mode-summary-card"><b>這個模式在做什麼</b><span>${meta.handbook}</span></div>
    <div class="mode-summary-card"><b>本模式優先順序</b><span>${meta.focus}</span></div>
    <div class="mode-summary-card"><b>手機手冊怎麼用</b><span>${meta.exportUse}</span></div>
  </div>
...
function renderPhaseTabs(plan) {
  ...
  let lastPhase = null;
  plan.forEach(week => {
    if (!lastPhase || lastPhase.name !== week.phase) {
      lastPhase = { name: week.phase, label: week.phaseLabel, start: week.weekNum, end: week.weekNum };
  ...
  return `<div class="phase-tabs">${phases.map(phase => {
    const isCurrent = phase.name === currentPhase;
    const isDone = phase.end < currentWeek;
    return `<div class="phase-tab ${isCurrent ? 'current' : ''} ${isDone ? 'done' : ''}" onclick="jumpToPhaseWeek(${phase.start})">
      ${emojiMap[phase.name] || '📍'} ${phase.label} W${phase.start}${phase.end > phase.start ? '–' + phase.end : ''}
    </div>`;
  }).join('')}</div>`;
}
...
<div class="week-nav">
  ...
  <div class="phase-explainer">${phaseRuleText}</div>
</div>
<div class="guide-actions">
  <button class="guide-chip" onclick="openGuideLibrary('warmup')">熱身圖解</button>
  <button class="guide-chip" onclick="openGuideLibrary('cooldown')">收操伸展</button>
  <button class="guide-chip" onclick="openGuideLibrary('strength')">肌力動作</button>
</div>
<div class="week-calendar">${week.days.map(day => renderDayCard(day)).join('')}</div>`;
}
...
function openGuideLibrary(kind) {
  ...
  const body = `
    ...
    <div class="guide-asset-stack">
      ${(guide.assets || []).map(asset => `
        <div class="guide-asset-card">
          <div class="guide-asset-title">${asset.title}</div>
          ${(asset.items || []).length ? `<ol class="guide-action-list">${asset.items.map(item => `<li>${item}</li>`).join('')}</ol>` : ''}
          ${asset.figures && asset.figures.length ? `<div class="guide-figure-grid">${asset.figures.map(([kind, label]) => `<div class="guide-figure-item">${renderExerciseFigure(kind)}<div class="guide-figure-label">${label}</div></div>`).join('')}</div>` : `<img class="guide-asset-image" src="${asset.src}" alt="${asset.alt || asset.title}">`}
          ${asset.caption ? `<div class="guide-asset-caption">${asset.caption}</div>` : ''}
        </div>
      `).join('')}
    </div>`;
...
function renderDayCard(day) {
  if (day.type === 'rest') {
    return `<div class="day-card ${day.status === 'missed' ? 'missed-card' : ''}">
      ...
      ${renderSupportCards(day.supportBlocks)}
    </div>`;
  }
...
function markDone(dateStr, type, plannedKm) {
  showModal(
    ...
    `<div>
      ...
      <div class="form-group"><label class="form-label">完成時間 (分鐘)</label><input class="form-input" id="m-time" type="number" placeholder="例：45"></div>
      <div class="form-group"><label class="form-label">體感強度 RPE (1–10)</label><input class="form-input" id="m-rpe" type="number" min="1" max="10" placeholder="6"><div class="field-help">RPE = 你主觀覺得有多累。3–4 很輕鬆、5–6 穩定可持續、7–8 吃力但可控制、9–10 幾乎全力。</div></div>
...
function markMissed(dateStr) {
  ...
  const plan = appData.plan || [];
  const today = todayStr();
  const weekIdx = plan.findIndex(week => week.days.some(day => day.dateStr === dateStr));
  if (weekIdx < 0) {
    renderPlanView();
    showView('plan');
    return;
  }
  const reschedCandidates = plan[weekIdx].days.filter(day => day.type === 'rest' && day.dateStr > today && day.dateStr > dateStr);
    ...
    [
      {
        ...
        primary: true,
        action: () => {
          const origDay = plan[weekIdx].days.find(day => day.dateStr === dateStr);
          ...
          reschedDay.supportBlocks = null;
          reschedDay.weekNum = origDay?.weekNum || weekIdx + 1;
          reschedDay.phaseName = origDay?.phaseName || plan[weekIdx].phase;
          saveData(appData);
          closeModal();
          renderPlanView();
          showView('plan');
...
function renderCheckinSection() {
  ...
  return `<div class="card">
    ...
    <p style="font-size:14px;color:var(--c-text-muted);margin-bottom:16px">這裡是主要檢查入口。一般情況下，不需要每天手動填一大堆資料；每週做一次這份檢查，再搭配課表卡上的完成/跳過，就夠系統判斷要不要調整。</p>
...
function adjustNextWeek(factor, removeInterval) {
  ...
  nextWeekPlan.days = nextWeekPlan.days.map(day => {
    if (removeInterval && day.type === 'interval') {
      return buildDayCard(day.dow, day.dateStr, 'easy', Math.round((day.km || 0) * factor * 10) / 10, appData.profile, false, false, !appData.profile.injuries.includes('none'), todayStr(), day.weekNum || currentWeek + 1, day.phaseName || nextWeekPlan.phase);
    }
...
function submitCheckin() {
  const score = CHECKIN_QUESTIONS.filter((_, index) => document.getElementById(`cq-${index}`)?.checked).length;
  ...
  appData.checkins = appData.checkins || [];
  appData.checkins.push({ weekNum: currentWeek, score, result, adjustment, date: todayStr(), fatigue, note });
  saveData(appData);
  assessProgress();
...
function assessProgress() {
  ...
  const plannedKm = plan.slice(0, currentWeek - 1).reduce((sum, week) => sum + week.targetKm, 0);
  const genDate = new Date(profile.generatedAt);
  const actualKm = log.filter(entry => {
    const weekNum = Math.floor((new Date(entry.date) - genDate) / 86400000 / 7) + 1;
    return weekNum < currentWeek;
  }).reduce((sum, entry) => sum + (entry.actualKm || 0), 0);
  const progressRate = plannedKm > 0 ? actualKm / plannedKm : 1;
  const plannedSessions = plan.slice(0, currentWeek - 1).reduce((sum, week) => sum + week.days.filter(day => day.type !== 'rest').length, 0);
  const doneSessions = plan.slice(0, currentWeek - 1).flatMap(week => week.days).filter(day => day.status === 'done').length;
  const adherenceRate = plannedSessions > 0 ? doneSessions / plannedSessions : 1;
  const recentLog = log.filter(entry => {
    const weekNum = Math.floor((new Date(entry.date) - genDate) / 86400000 / 7) + 1;
    return weekNum >= currentWeek - 2 && weekNum < currentWeek;
  });
...
function resumePlan() {
  ...
  appData.plan.forEach(week => {
    week.days.forEach(day => {
      if (day.dateStr && new Date(day.dateStr) >= pausedAt && day.status === 'upcoming') {
        ...
        day.dateStr = shifted.toISOString().split('T')[0];
      }
    });
  });
  ...
  appData.profile.targetDate = newTargetDate.toISOString().split('T')[0];
  saveData(appData);
  renderPlanView();
  showView('plan');
}
...
function rebuildWeeksFrom(startWeekNum, count) {
  ...
  for (let wi = 0; wi < count; wi++) {
    ...
    const preserved = week.days.filter(day => day.status === 'done' || day.status === 'missed');
...
function renderLogSection() {
  ...
  const totalKm = log.reduce((sum, entry) => sum + (entry.actualKm || 0), 0);
  const plan = appData.plan || [];
  const plannedSessions = plan.reduce((sum, week) => sum + week.days.filter(day => day.type !== 'rest').length, 0);
  ...
  const rows = [...log].reverse().map(entry => {
    ...
    const typeName = { easy: '輕鬆跑', tempo: '節奏跑', interval: '間歇跑', long: '長跑' }[entry.type] || entry.type;
    return `<tr>
      <td>${entry.date}</td>
      <td>${typeName}</td>
