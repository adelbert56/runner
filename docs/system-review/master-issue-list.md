...
| SEC-002 | 資安／架構 | P1 | 已實作：正式 Pages HTTPS 來源在呼叫本機 Garmin 課表 API 前，必須帶入本機產生的 192-bit 配對碼；配對碼僅能由 loopback 頁面讀取，公開頁只以 sessionStorage 暫存。未配對讀取／同步回 `401`，公開來源讀取配對碼回 `403`。 | 本機 Garmin 課表發布 | 保留橋接；若未來需要跨裝置或長效授權，再升級為可撤銷的裝置配對管理。 | 已修復（本機驗證） | 未配對公開來源無法讀取狀態或啟動同步；帶正確碼的正式 Pages HTTPS 來源可讀取狀態；配對碼 API 不接受公開來源。 |
