...
| 2026-08-24 | Registration workflow A: required-profile completion summary plus inline three-step entry guidance | Registration people and entries | Collapsed required profile details, unclear post-race next action, and payment state that could contradict registration |
| 2026-07-27 | Day cards use one compact prescription strip for distance, heart-rate target and pace; activity-derived metrics name their source and date | Trainer weekly plan and progress analysis | Dense plain-text metric rows and device-like labels for activity-only values |
