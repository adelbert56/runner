...
assertCheck(/function earlyCoachPlanningEligibility\(/.test(trainer) && /day\.type !== 'rest' && !day\.isMakeup/.test(trainer) && /garminRunsByDate/.test(trainer) && /activityCompletesDay\(day, garminRunsByDate\.get\(day\.dateStr\)\)/.test(trainer) && /function openEarlyCoachPlanning\(/.test(trainer) && /function submitEarlyCoachPlanning\(/.test(trainer) && /我已完成，手動確認排課/.test(trainer) && /manualCompletionConfirmed/.test(trainer) && /earlyTrigger/.test(trainer), "coach advice counts matched Garmin sessions and offers a per-course manual confirmation when historical Garmin dates cannot match the current plan");
