...
assertCheck(/function earlyCoachPlanningEligibility\(/.test(trainer) && /day\.type !== 'rest' && !day\.isMakeup/.test(trainer) && /function openEarlyCoachPlanning\(/.test(trainer) && /function submitEarlyCoachPlanning\(/.test(trainer) && /手動提前排定下週/.test(trainer) && /earlyTrigger/.test(trainer), "coach advice allows a guarded manual early next-week planning trigger after every scheduled running session is complete");
