...
assertCheck(/function workoutStructureForDay\(/.test(trainer) && /function coachWorkoutStructure\(/.test(trainer) && /查看 Garmin 課程結構/.test(trainer) && !/<details class="coach-jargon" open/.test(trainer) && /每一組依序完成/.test(trainer) && /workoutStructure: coachWorkoutStructure/.test(trainer) && /hasCurrentCoachPlan/.test(trainer) && /查看歷史教練週報/.test(trainer), "coach Garmin structure stays collapsible and historical reviews do not expand into the current-week plan");
...
assertCheck(/hasUpcomingCoachPlan/.test(trainer) && /下週教練調整後的課程/.test(trainer) && /查看歷史教練週報/.test(trainer), "future coach reviews render as directly usable next-week plans while completed reviews remain historical");
...
assertCheck(/function courseRationale\(day, ctx\)/.test(trainer) && /class="course-rationale"/.test(trainer) && /renderDayCard\(resolved\.course, resolved\.rationale\)/.test(trainer), "day cards expose the resolver rationale instead of inventing a separate decision explanation");
assertCheck(/function renderCourseRationaleSummary\(/.test(trainer) && /本週課程調整依據/.test(trainer) && /完成度／提醒／自動決策已各自固定/.test(trainer), "coach tab summarizes resolved adjustments while progress avoids duplicating week status cards");
