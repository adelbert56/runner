...
assertCheck(/function isCalibrationSafeRun\(/.test(trainer) && /temperatureC\) >= 30/.test(trainer) && /function coachPrescriptionLocksWeek\(/.test(trainer) && /function applyCourseSpacingGuard\(/.test(trainer) && /安全保護高於教練處方/.test(trainer), "coach calibration excludes heat and terrain while safety overrides coach prescriptions and protects recovery spacing");
