...
assertCheck(/趨勢待建立：最近 14 天/.test(trainer) && /同課型各需 2 筆/.test(trainer) && !/前期 \$\{.*?\}\/2/.test(trainer), "Garmin comparison evidence labels two 14-day windows instead of presenting counts as a completion ratio");
assertCheck(/comparisonFamily/.test(trainer) && /同課型各需 2 筆/.test(trainer) && /day\.dateStr\.slice\(5\)\.replace\('-', '\/'\)/.test(trainer), "Garmin Autopilot compares only matching workout families and shows menu dates");
...
assertCheck(/function updatePlanInPlace\(/.test(trainer) && /profile\.generatedAt = oldProfile\.generatedAt \|\| profile\.generatedAt/.test(trainer) && /已完成紀錄、過去課表與賽事安排都會保留/.test(trainer) && !/重新生成會取代目前計畫/.test(trainer), "updating handbook settings reflows only future sessions instead of resetting the whole plan");
