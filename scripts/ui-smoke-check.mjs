...
assertCheck(/function earlyCoachPlanningEligibility\(/.test(trainer) && /day\.type !== 'rest' && !day\.isMakeup/.test(trainer) && /garminRunsByDate/.test(trainer) && /activityCompletesDay\(day, garminRunsByDate\.get\(day\.dateStr\)\)/.test(trainer) && /function garminCoachMenuForNextWeek\(/.test(trainer) && /function applyGarminCoachPlanForNextWeek\(/.test(trainer) && /Garmin 實跑後的下週調整/.test(trainer) && /garminCoachAppliedFor/.test(trainer) && /套用教練調整並查看下週/.test(trainer) && !/function submitEarlyCoachPlanning\(/.test(trainer), "coach advice turns completed Garmin sessions into an adjustable next-week plan instead of merely revealing the original schedule");
