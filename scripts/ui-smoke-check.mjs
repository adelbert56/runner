...
assertCheck(/function workoutStructureForDay\(/.test(trainer) && /function coachWorkoutStructure\(/.test(trainer) && !/<details class="coach-jargon" open/.test(trainer) && /每一組依序完成/.test(trainer) && /查看歷史教練週報/.test(trainer) && !/function renderLiveCoachCard\(/.test(trainer), "Garmin workout structures remain supported without expanding historical reviews into a second schedule");
...
assertCheck(/function preserveCoachWorkoutSteps\(/.test(garminReviewBuilder) && /function normalizeCoachSteps\(/.test(garminReviewBuilder) && /function refreshReviewFromGarmin\(/.test(garminReviewBuilder) && /updatedAt: asOf/.test(garminReviewBuilder) && /weekStart: hasCurrentManualMenu \? previousMenuStart : nextStart/.test(garminReviewBuilder) && /previousMenuStart >= currentStart/.test(garminReviewBuilder), "encrypted coach review preserves structured steps and keeps a manual menu until its week is over");
...
assertCheck(/function heroEasyPaceInfo\(/.test(trainer) && /function coachEasyHrCap\(/.test(trainer) && /adaptiveEasyPaceSec\(profile, new Date\(\)\)/.test(trainer) && /function menuPaceDriftNote\(/.test(garminReviewBuilder) && /slowestPrescribed - actual/.test(garminReviewBuilder), "easy pace is governed by heart rate and calibrated from recent runs, with stale prescribed paces flagged");
assertCheck(/function dedupePlanChangeItems\(/.test(trainer) && /function planChangeSummary\(/.test(trainer) && /coach-evidence-group-title/.test(trainer), "plan change history is deduplicated, date-ordered, and grouped in the coach evidence panel");
assertCheck(/children: step\.children\.map\(withSpec\)/.test(trainer) && /function warnMenuWithoutSteps\(/.test(garminReviewBuilder), "repeat-group children carry Garmin target specs and menus without structured steps are flagged at build time");
assertCheck(/function formalCoachFallbackMenu\(/.test(trainer) && /const upcomingWeekStart = appData\.plan\?\.\[currentWeek\]\?\.days\?\.\[0\]\?\.dateStr/.test(trainer) && /查看未來教練週報（待對應）/.test(trainer), "older coach reviews stay evidence-only until their week matches the formal plan");
assertCheck(/function liveCoachPlan\(/.test(trainer) && /const hasNewerGarminRuns/.test(trainer) && /run\.date >= reviewedPlanStart/.test(trainer) && /function renderCoachDecisionWorkspace\(/.test(trainer), "recent Garmin records remain part of the shared coach decision input");
...
assertCheck(/function renderCoachDecisionWorkspace\(/.test(trainer) && /resolveWeeklyDecision\(context, plan\[currentWeek - 1\]\)/.test(trainer) && /正式課程只在「本週課表」呈現/.test(trainer), "coach page reads the same weekly resolver and keeps official courses on the week tab");
...
assertCheck(/function garminLoadDecision\(/.test(trainer) && /trainingLoad[\s\S]*aerobicTe[\s\S]*anaerobicTe/.test(trainer) && /下週跑量下修/.test(trainer) && /const loadDecision = garminLoadDecision\(completedRuns\)/.test(trainer) && /本週已同步/.test(trainer) && /function recordGarminAnalysisSnapshot\(/.test(trainer), "sustained Garmin load and training effect safely reduce future volume while preserving single-run guardrails and a visible analysis snapshot");
assertCheck(/hasUpcomingCoachPlan/.test(trainer) && /查看歷史教練週報/.test(trainer) && /查看未來教練週報（待對應）/.test(trainer), "future coach reviews remain traceable evidence without rendering a second schedule");
...
assertCheck(/let coachReviewLoadState = 'loading'/.test(trainer) && /正在核對 Garmin 已同步紀錄/.test(trainer) && /先解鎖教練建議/.test(trainer), "early planning waits for Garmin review data instead of falsely reporting every session incomplete before coach review is ready");
...
assertCheck(!/plan-pulse-summary/.test(trainer) && !/function renderProgressCard\(/.test(trainer) && /\$\{s\.weekProgressPct\}%/.test(trainer), "week overview shows weekly volume once instead of a separate plan-pulse card duplicating hero context");
assertCheck(/function renderCourseDecisionPanel\(/.test(trainer) && /本週排課調整/.test(trainer) && !/\$\{renderDailyExecutionCard\(week\)\}/.test(trainer) && /class="course-decision-panel"/.test(trainer), "week view keeps one concise decision owner without duplicate daily guidance");
...
assertCheck(/function plannedSessionFor\(run\)/.test(trainer) && /function resolveCourse\(day, ctx/.test(trainer) && /resolveCourse\(day, buildContext\(\), week\)\.course/.test(trainer), "session report uses the same resolver output as the plan and today card");
assertCheck(/function courseRationale\(day, ctx\)/.test(trainer) && /function courseResolutionLabel\(source\)/.test(trainer) && /class="course-rationale"/.test(trainer) && /renderDayCard\(resolved\.course, resolved\.rationale, resolved\.source\)/.test(trainer) && /function renderCourseDecisionPanel\(/.test(trainer) && /本週先照哪個版本跑？/.test(trainer) && /course-decision-focus/.test(trainer), "week view turns resolver output into a compact next-course decision instead of repeating every day");
assertCheck(!/function renderCourseRationaleSummary\(/.test(trainer) && !/function renderLiveCoachCard\(/.test(trainer) && /class="coach-evidence"/.test(trainer) && /查看資料依據與變更紀錄/.test(trainer), "coach tab collapses calibration and Garmin evidence instead of duplicating a weekly menu");
...
assertCheck(/function recordPlanChange\(/.test(trainer) && /function renderPlanChangeTimeline\(/.test(trainer) && /Garmin 實跑自動校準/.test(trainer) && /previous\.source === item\.source && previous\.title === item\.title && previous\.changes\.join\('\|'\) === item\.changes\.join\('\|'\)/.test(trainer), "automatic plan changes collapse duplicate Garmin calibration results across sync days");
...
assertCheck(/function postRunVerdict\(/.test(trainer) && /function trainingAutopilotDecision\(/.test(trainer) && /自動訓練決策/.test(trainer) && /缺口不會硬塞到明天/.test(trainer), "post-run completion, safe makeup, and next-step guidance use one automatic decision layer");
...
assertCheck(/function isCalibrationSafeRun\(/.test(trainer) && /function heatAdjustedPaceSec\(/.test(trainer) && /temperatureC\) >= 35/.test(trainer) && /function coachPrescriptionLocksWeek\(/.test(trainer) && /function applyCourseSpacingGuard\(/.test(trainer) && /function safetyGuard\(day, ctx\)/.test(trainer) && /source: 'safety-hold'/.test(trainer), "coach calibration normalizes heat instead of freezing all summer, excludes terrain and extreme heat, and safety overrides coach prescriptions");
...
assertCheck(/function runCoachAdaptation\(trigger, options/.test(trainer) && /runCoachAdaptation\('coach-review-ready'\)/.test(trainer) && /runCoachAdaptation\('weekly-checkin', decision\)/.test(trainer), "persistent coach adaptations enter through one orchestrator while course resolution stays render-time");
assertCheck(/function dailyAdjust\(day, ctx\)/.test(trainer) && /function progression\(ctx, trigger, options/.test(trainer) && /function paceResolver\(ctx, date/.test(trainer) && /paces: paceResolver\(ctx, day\.dateStr\)/.test(trainer), "daily adjustment, progression, and pace source are explicit adapters while legacy mutations retain their verified write paths");
...
assertCheck(!/function weeklyCoachLetterBody\(/.test(trainer) && /function resolveWeeklyDecision\(/.test(trainer) && /function renderCourseDecisionPanel\(/.test(trainer) && !/week-guidance/.test(trainer) && !/week-coach-brief/.test(trainer), "week tab has one decision owner instead of a coach letter and duplicate guidance cards");
assertCheck(/function normalizeTrainingCheckins\(/.test(trainer) && /function renderCheckinHistory\(/.test(trainer) && /歷史評估/.test(trainer) && /checkin-history-item/.test(trainer) && /filter\(\(item\) => item\.weekNum !== currentWeek\)/.test(trainer), "weekly review keeps one latest decision per week and shows prior decisions");
