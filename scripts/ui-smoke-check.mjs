...
assertCheck(/GARMIN_WORKOUT_PAIRING_KEY/.test(trainer) && /sessionStorage\.getItem\(GARMIN_WORKOUT_PAIRING_KEY\)/.test(trainer) && /sessionStorage\.setItem\(GARMIN_WORKOUT_PAIRING_KEY, code\)/.test(trainer) && /openGarminWorkoutPairing/.test(trainer) && /x-runner-garmin-pairing/.test(trainer) && /api\/garmin-workout-pairing/.test(server) && /randomBytes\(24\)/.test(server) && /timingSafeEqual/.test(server) && /pairing-required/.test(server) && /isLocalGarminActivitySyncRequest\(req\)/.test(server), "public Garmin workout bridge requires a session pairing code that only a loopback page can retrieve");
...
assertCheck(/function earlyCoachPlanningEligibility\(/.test(trainer) && /day\.type !== 'rest' && !day\.isMakeup/.test(trainer) && /garminRunsByDate/.test(trainer) && /activityCompletesDay\(day, garminRunsByDate\.get\(day\.dateStr\)\)/.test(trainer) && /function openEarlyCoachPlanning\(/.test(trainer) && /function submitEarlyCoachPlanning\(/.test(trainer) && /手動提前排定下週/.test(trainer) && /earlyTrigger/.test(trainer), "coach advice counts matched Garmin sessions when deciding whether early next-week planning can unlock");
